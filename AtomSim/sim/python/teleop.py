...
def world_to_screen(x: float, y: float) -> tuple[int, int]:
    ...
    return (int(WINDOW_W / 2 + x * SCALE_PX_PER_M),
...
def _rotate_translate(pt: tuple[float, float],
                       theta: float,
                       ox: float, oy: float) -> tuple[float, float]:
    c, s = np.cos(theta), np.sin(theta)
    return (ox + c * pt[0] - s * pt[1],
            oy + s * pt[0] + c * pt[1])
...
def draw_robot(surf, robot):
    ...
    chassis_screen = [world_to_screen(*_rotate_translate(p, theta, px, py))
                      for p in chassis_local]
    ...
    pygame.draw.line(surf, ROBOT_OUTLINE_COLOR,
                     world_to_screen(px, py),
                     world_to_screen(*nose_world), 2)
...
def load_robot_config(name: str) -> sim_py.RobotConfig:
    ...
    if "manipulator" in data:
        m_data = json.loads((cfg_dir / "manipulators" / f"{data['manipulator']}.json").read_text())
...
def main() -> None:
    ...
    # --- control mapping ---
    MAX_WHEEL_SPEED = 0.6   # m/s — caps wheel rim velocity at full key press
    DT              = 1.0 / 60.0

    def reset() -> None:
        rs = np.array([robot_cfg.x0, robot_cfg.y0, robot_cfg.theta0, 0.0, 0.0],
                      dtype=np.float32)
        bs = np.array([ball_cfg.x0,  ball_cfg.y0,  0.0, 0.0],
                      dtype=np.float32)
        robot.set_state(rs)
        ball.set_state(bs)
    ...
    while running:
        ...
        forward = (keys[pygame.K_w] or keys[pygame.K_UP])    - (keys[pygame.K_s] or keys[pygame.K_DOWN])
        turn    = (keys[pygame.K_a] or keys[pygame.K_LEFT])  - (keys[pygame.K_d] or keys[pygame.K_RIGHT])
        v_left  = MAX_WHEEL_SPEED * (forward - turn)
        v_right = MAX_WHEEL_SPEED * (forward + turn)
