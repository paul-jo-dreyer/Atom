"""Train a PPO policy on the Level-1 SoloEnv with `torch.compile` applied
to the PPO policy submodules.

This is a parallel script to `train.py`. The training logic (rewards,
PPO hyperparameters, callbacks, resume, GIF eval, vec env transport)
is identical. The only differences are:
  * After PPO construction (or load), the policy's MLP submodules are
    wrapped via `torch.compile()`. PyTorch 2.x JIT-compiles the forward
    graph into fused kernels, which on CPU can speed up the small-batch
    matmul-heavy update phase by 20-40% for our network size.
  * On first call, compile incurs a one-time cost (~seconds). FPS
    measurements should drop the first few iterations as warmup.

Caveats:
  * `torch.compile` may break under odd edge cases. Default
    `mode="default"` is the most stable; `mode="reduce-overhead"`
    is GPU-specific (CUDA graphs); `mode="max-autotune"` would
    give marginally more speed for compile-time cost.
  * Resume from compile-trained checkpoints should work because
    `torch.compile`'s OptimizedModule passes `state_dict()` through
    to the underlying module, but it's untested in our codebase.
  * If compile fails, this script is louder about it than train.py.

Usage (run from the repo root, with the venv active):

    .venv/bin/python -m AtomGym.training.train_compile --run-name l1_compile
"""
...
def main() -> None:
    ...
    # === torch.compile the policy submodules ===
    # Compile the MLP submodules used in the hot loop (forward during
    # rollout, forward+backward during update). torch.compile traces
    # the graph, fuses ops, and emits optimized kernels. For CPU MLPs
    # the typical win is 20-40% on update phase wall-clock.
    #
    # We compile submodules rather than the whole policy so we don't
    # touch the dist sampling / log_prob / entropy code paths (those
    # don't benefit from compile and are more likely to trigger weird
    # edge cases). The matmul-heavy parts are policy_net, value_net
    # (in mlp_extractor), action_net, and value_net (heads).
    import torch
    print("[train] compiling policy submodules (torch.compile)…")
    model.policy.mlp_extractor.policy_net = torch.compile(
        model.policy.mlp_extractor.policy_net
    )
    model.policy.mlp_extractor.value_net = torch.compile(
        model.policy.mlp_extractor.value_net
    )
    model.policy.action_net = torch.compile(model.policy.action_net)
    model.policy.value_net = torch.compile(model.policy.value_net)
    print("[train] compile setup complete (first forward will JIT, ~few seconds)")
